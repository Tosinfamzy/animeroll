  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col overflow-hidden">
        {/* Mount the body only when open so useState initializers run fresh
            each time and we avoid setState-in-effect to "reset" stale form state. */}
        {open ? <EntryDetailBody entry={entry} anime={anime} listIds={listIds} /> : null}
      </DialogContent>
    </Dialog>
  );
}
...
function EntryDetailBody({
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      return jsonFetch<{ data: unknown }>(`/api/entries/${entry.id}`, {
        method: 'PATCH',
  ...
  const refresh = useMutation({
    mutationFn: () =>
      jsonFetch<{ data: unknown }>(`/api/anime/${anime.malId.toString()}`, {
        method: 'POST',
      }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['entries'] });
