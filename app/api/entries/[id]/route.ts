...
export async function PATCH(req: Request, { params }: RouteCtx) {
  ...
  const rl = await checkRateLimit(userKeyFromRequest(userId, 'entries-patch'), 120, 60_000);
  if (!rl.allowed) {
...
export async function DELETE(req: Request, { params }: RouteCtx) {
  ...
  const rl = await checkRateLimit(userKeyFromRequest(userId, 'entries-delete'), 30, 60_000);
  if (!rl.allowed) {
